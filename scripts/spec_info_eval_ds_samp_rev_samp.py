...
Note: Conduct all essential physical examinations (PE), laboratory tests (LAB), microbiological tests (MICRO), and imaging studies (IMAGE) to ensure a thorough diagnosis. When you are confident, choose the "OUTPUT" action and you will be asked to output the corresponding diagnosis and treatment plan.
...
Diagnosis: (all the possible diagnosis, specific disease name)
...
def chating_child(args, repeat=0):
    # try:
    message, model = args[0], args[1]
    while True:
        try:
            s_time = time.time()
            ...
            if 'filter' in reply.choices[0].finish_reason:
                return '请求超时/内容过滤'
            reply = reply.choices[0].message.content
            e_time = time.time()
            print("time spent {}".format(e_time-s_time))
            break
...
def action_parser(response):
    ...
    if 'Diagnosis:' in response and 'Treatment:' in response:
        ...
        treatment_patterns = [r"Treatment:(.*)\Z",r"Treatment Plan:(.*)\Z"]
...
def generate(prompts, model,n_process=4):
    inputs = [[p,model] for p in prompts]
    with Pool(processes=n_process) as pool:
        replys = pool.map(chating_child, inputs)
    return replys

def post_processing(ids, orig_response, ids_chat_history, ids_action_history, ids_resources, ids_test_set, exam_name_mapping, exam_spec_mapping,lab_test_id_to_name, itemid_to_name):
    ids_lab_event_request, ids_image_event_request, ids_micro_event_request = [], [], []
    finish = False
    result, finish, response = action_parser(orig_response)
    if finish:
        ids_chat_history.append({'role': 'assistant', 'content': orig_response})    # ends with assistant
        ...
        ids_chat_history.append({'role': 'assistant', 'content': orig_response})
...
def main(args):
    # 1. Load Model
    # model = LLM(model=args.model,tensor_parallel_size=args.num_cuda, gpu_memory_utilization=0.98,max_model_len=args.max_length,trust_remote_code=True, swap_space=32)
    ...
    microbiologyevents = pd.read_csv(os.path.join('/hdd/zhouyx/mimic-iv-2.2/hosp', 'microbiologyevents.csv'),low_memory=False)
    ...
    while len(valid_ids) > 0:
        ...
        # summarize_ids = []
        for ids in valid_ids:
            input_msgs.append(chat_history[ids])
        responses = generate(input_msgs, args.model, args.num_workers)
        new_ids = []
        for ids, response in zip(valid_ids, responses):
            if response == '请求超时/内容过滤' or response == '请求超时':
                chat_history[ids].append({'role': 'assistant', 'content': response})
                results[ids] = [response, response]
            else:
                new_ids.append(ids)
        valid_ids = new_ids
        new_ids = []
        ...
        batch_results = pool.starmap(post_processing, zip(valid_ids, responses, [chat_history[ids] for ids in valid_ids], [action_history[ids] for ids in valid_ids], [resources[ids] for ids in valid_ids], [test_set[ids] for ids in valid_ids], [exam_name_mapping]*len(valid_ids), [exam_spec_mapping]*len(valid_ids), [lab_test_id_to_name]*len(valid_ids), [itemid_to_name]*len(valid_ids)))
    ...
    if args.seed == 0:
        dir_name = 'results_part'
    else:
        dir_name = f"results_part_{args.seed}"
    if not os.path.exists(dir_name):
        os.makedirs(dir_name)
    pickle.dump(results, open(f"{dir_name}/results_{args.model_name}_rev.pkl",'wb'))
    pickle.dump(chat_history, open(f"{dir_name}/chat_history_{args.model_name}_rev.pkl",'wb'))
    pickle.dump({'lab': lab_event_request, 'micro': micro_event_request, 'image': image_event_request}, open(f"{dir_name}/event_request_{args.model_name}_rev.pkl",'wb'))
