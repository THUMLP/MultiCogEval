...
from dataset.discharge import (
    extract_history,
    extract_history_general,
    extract_physical_examination,
    extract_physical_examination_general,
...
def extract_hadm_ids_general(pathology, diag_icd, discharge_df, diag_counts=20, cc=10):
    # Grab all hadm_ids with appendicitis
    hadm_ids = (
        diag_icd[diag_icd["long_title"].str.contains(pathology, case=False)][
            ["hadm_id"]
...
def extract_info_general(
    ...
    # Extract the discharge, history, pe, le and radiology report for hadm_ids
    hadm_info = extract_hadm_info_general(
        pathology,
        hadm_ids,
        discharge_df,
        admissions_df,
    ...
    try:
        ...
        # Examine data completeness
        hadm_info_clean = check_missing_general(hadm_info, pathology)
        print("--")
...
def process_id_chunk(chunk_ids, lab_events_df, radiology_reports_df, microbiology_df, transfers_df, hadm_to_subject_id):
    le_mask_list, rad_mask_list, micro_mask_list = [], [], []
    for _id in chunk_ids:
        ...
        le_mask_list.append([_id, mask_le])
        # lab_events_df.loc[mask_le, "hadm_id"] = _id
        ...
        rad_mask_list.append([_id, mask_rad])
        # radiology_reports_df.loc[mask_rad, "hadm_id"] = _id
        ...
        micro_mask_list.append([_id, mask_micro])
    return le_mask_list, rad_mask_list, micro_mask_list

def fill_nan_hadm_optimized(
    lab_events_df,
    radiology_reports_df,
    microbiology_df,
    disease_ids,
    ...
    chunks = np.array_split(disease_ids, 8)
    pool = Pool(8)
    results = pool.starmap(process_id_chunk, [(chunk, lab_events_df, radiology_reports_df, microbiology_df, transfers_df, hadm_to_subject_id) for chunk in chunks])
    pool.close()
    pool.join()
    # 批量更新
    for le_mask_list, rad_mask_list, micro_mask_list in tqdm(results):
        if le_mask_list:
            for _id, mask_le in tqdm(le_mask_list):
                lab_events_df.loc[mask_le, 'hadm_id'] = _id

        if rad_mask_list:
            for _id, mask_rad in tqdm(rad_mask_list):
                radiology_reports_df.loc[mask_rad, 'hadm_id'] = _id

        if micro_mask_list:
            for _id, mask_micro in tqdm(micro_mask_list):
                microbiology_df.loc[mask_micro, 'hadm_id'] = _id
...
def extract_hadm_info_general(
    ...
    # Drop hadm without disease in the discharge diagnosis
    filtered_disease_ids = []
    for _id in disease_ids:
        if _id in discharge_dict:
            discharge_row = discharge_dict[_id]
            discharge_text = discharge_row["text"]
            try:
                discharge_diagnosis = extract_diagnosis_from_discharge(discharge_text)
            except Exception as e:
                discharge_diagnosis = ""
            if pathology_in_primary_diagnosis(pathology.lower(), discharge_diagnosis.lower()):
                filtered_disease_ids.append(_id)
    print("There are {} hadm_ids with {} in the discharge diagnosis".format(len(filtered_disease_ids), pathology))
    disease_ids = np.asarray(filtered_disease_ids)
    # Fill in NaN hadm_ids if possible
    (
        lab_events_df_sf,
        radiology_report_df_sf,
    ...
    for _id in disease_ids:
        if _id in discharge_dict:
            ...
            history = extract_history_general(discharge_text)

            pe = extract_physical_examination_general(discharge_text)
...
def check_missing_general(hadm_info, pathology):
    print("There are {} patients.".format(len(hadm_info)))
    missing = 0
    missing_history = 0
    missing_pe = 0
